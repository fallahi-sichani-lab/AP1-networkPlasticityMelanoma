...
bzip_names = pd.read_table('Human_bZip.csv',sep = ",")
all_genes = pd.DataFrame({"Gene":df.columns[9:]})
bzip_names = bzip_names[bzip_names["Name1"].isin(all_genes["Gene"])]
bzip_names["Name1"]

selected_ap1s = ["FOS","FOSL1","FOSL2","JUN","JUNB","JUND","ATF2","ATF4"]

bzip_no_ap1_names = list(set(bzip_names["Name1"].to_list())-set(ap1_names))
...
from sklearn.metrics import SCORERS, mean_squared_error, r2_score, explained_variance_score
...
def run_plsr(df,selected_factors,seed_id,n_splits,val_size):
    ...
    X = df_selected.drop(metadata_columns,axis = 1,inplace=False) #PLSR input: gene expression of selected factors
    Y = df_selected[metadata_columns[2:]] #PLSR output: signature z-scores 
    # Create Pipeline
...
def myf(_):
    ...
    selected_factors = np.array(bzip_no_ap1_names)[np.asarray(random_combination(range(n), r))]
...
sns.distplot(df_results[metrics].to_numpy(),color='xkcd:azure')
...
plt.xlabel(metrics)
plt.xlim(-1,1)
plt.title("bZIP PLSR Model Performance Distribution")
plt.savefig(f'bZIP_PLSR_Model_{metrics}_Distribution.pdf', format='pdf')
